void quick(int xBin = 8, int QQBin = 4)
{
gStyle->SetOptStat(0);
...
{
can->cd(k+1);
hist[k] = (TH2F*) tf->Get(Form("rec_pip_PT2vsz_x%iQQ%iphihB%i", xBin, QQBin, k));
hist[k]->GetXaxis()->SetTitle("z");
hist[k]->GetYaxis()->SetTitle("P_{h#perp}^{2} (GeV^{2})");
hist[k]->SetTitle(Form("x%i QQ%i, %3.1f#circ < |#phi_{h}| < %3.1f#circ", xBin, QQBin, k*2.0, (k+1)*2.0)); // 2.0 is the bin width
hist[k]->Draw("colz");
