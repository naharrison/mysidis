...
// uses "update" instead of "recreate" in TFile to add histos one by one... loop over all bins must be done externally
...
void systematics_v2(int xBin = 0, int QQBin = 0, int zBin = 3, int PT2Bin = 0, string pipORpim = "pip")
{
gStyle->SetOptStat(0);
...
string MString = Form("hM_sysEcontributions_%s_%i_%i_%i_%i", pipORpim.c_str(), xBin, QQBin, zBin, PT2Bin);
string AcString = Form("hAc_sysEcontributions_%s_%i_%i_%i_%i", pipORpim.c_str(), xBin, QQBin, zBin, PT2Bin);
string AccString = Form("hAcc_sysEcontributions_%s_%i_%i_%i_%i", pipORpim.c_str(), xBin, QQBin, zBin, PT2Bin);
string catString = Form("hCategory_%s_%i_%i_%i_%i", pipORpim.c_str(), xBin, QQBin, zBin, PT2Bin);
TH1F *hM_sysEcontributions = new TH1F(MString.c_str(), MString.c_str(), Nsources, 0, Nsources);
TH1F *hAc_sysEcontributions = new TH1F(AcString.c_str(), AcString.c_str(), Nsources, 0, Nsources);
TH1F *hAcc_sysEcontributions = new TH1F(AccString.c_str(), AccString.c_str(), Nsources, 0, Nsources);
