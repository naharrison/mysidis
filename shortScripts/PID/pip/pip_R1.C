...
//const int Nstrictnesses = 1;
//float height[Nstrictnesses] = {10};
//float angle[Nstrictnesses] = {60};
...
{
can->cd(c*6 + s + 1);
hD[c][s] = (TH2F*) fD->Get(Form("pipIDplots/tl1_xVtl1_y_pip_hist_c%i_s%i", c, s+1));
...
can->cd(12 + c*6 + s + 1);
hMC[c][s] = (TH2F*) fMC->Get(Form("pipIDplots/tl1_xVtl1_y_pip_hist_c%i_s%i", c, s+1));
...
//can2->SaveAs(Form("pip_R1_s%i.png", sector));
