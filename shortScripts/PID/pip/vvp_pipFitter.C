void vvp_pipFitter(int sector = 1)
{
gStyle->SetOptStat(0);
...
hvvp[t] = (TH2F*) file[t]->Get(Form("pipIDplots/vvp_pip_esect_c1_s%i", sector));
...
slicecan[t]->Divide(10, 7, 0.00001, 0.00001); // might need manual change
   ...
   if(t==1) slice[t][a]->Fit(Form("sliceFF%i_t%i_s%i", a, t, sector), "q M", "", sliceFF[t][a]->GetParameter(1) - 0.012, sliceFF[t][a]->GetParameter(1) + 0.012);
	...
	if(t == 1)
	{
	if(a >= 25 && a <= 27) lowerCutVal[1][a] = 0.965;
	if(a >= 28 && a <= 30) lowerCutVal[1][a] = 0.970;
	if(a >= 31 && a <= 34) lowerCutVal[1][a] = 0.975;
	if(a >= 35 && a <= 999) lowerCutVal[1][a] = 0.977;
	}
	// end exceptions
	...
	upperCutVal[0][a] = sliceFF[0][a]->GetParameter(1) + recalcNsig_up*sliceFF[0][a]->GetParameter(2);
		...
		if(a >= 63) // don't want the cut to taper out at high p, if it tries to, these lines prevent it by setting the cut equal to the previous value
		{
		if(lowerCutVal[0][a] < lowerCutVal[0][a-1]) lowerCutVal[0][a] = lowerCutVal[0][a-1];
		if(upperCutVal[0][a] > upperCutVal[0][a-1]) upperCutVal[0][a] = upperCutVal[0][a-1];
		}
	}
...
upmarker_sysup->Draw();
	...
	//ofstream outfile;
	//outfile.open(Form("t%i_pipVelocityCut_es%i_pBin%i.txt", t, sector, a));
...
TH2F *notCorrData = (TH2F*) file[1]->Get(Form("pipIDplots/vvp_pip_esect_notCorr_s%i", sector));
notCorrData->GetXaxis()->SetTitle("p (GeV)");
notCorrData->GetYaxis()->SetTitle("velocity (c)");
notCorrData->SetTitle(Form("data, e- s%i, positive tracks", sector));
//TH2F *notCorrMC = (TH2F*) file[0]->Get(Form("pipIDplots/vvp_pip_esect_notCorr_s%i", sector));
//notCorrMC->GetXaxis()->SetTitle("p (GeV)");
//notCorrMC->GetYaxis()->SetTitle("velocity (c)");
//notCorrMC->SetTitle(Form("data, e- s%i, positive tracks", sector));

can2D->cd(1)->SetLogz();
notCorrData->Draw("colz");

TH2F *nocutsData = (TH2F*) file[1]->Get(Form("pipIDplots/vvp_pip_esect_c0_s%i", sector));
...
TH2F *nocutsMC = (TH2F*) file[0]->Get(Form("pipIDplots/vvp_pip_esect_c0_s%i", sector));
...
//slicecan[0]->SaveAs(Form("pip_vvp_MC_1D_s%i.png", sector));
//slicecan[1]->SaveAs(Form("pip_vvp_data_1D_s%i.png", sector));
//can2D->SaveAs(Form("pip_vvp_2D_s%i.png", sector));
